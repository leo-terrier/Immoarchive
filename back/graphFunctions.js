...
exports.generateGraphData = void 0;
const utils_1 = require("./utils");
const generateHistoStep = ({ deals, range, prop }) => {
    ...
    while (step === 0 && currentStep < range[1] + 1) {
        const value = smallestOfHighest[prop];
        if (value < currentStep * 9) {
            step = currentStep;
        }
        //Avoid non round step (ex: 750)
        currentStep = (0, utils_1.roundToLeadingDigit)(currentStep * 2);
    }
    return step;
};
const histoData = ({ deals, prop, range }) => {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (isRoomsHisto) {
            ...
            if (i !== 0 && i !== 9) {
                bar.tooltipProps.label = `${barValue.toLocaleString('fr-FR')} - ${(barValue + step).toLocaleString('fr-FR')}`;
                barValue += step;
            }
            else {
                const label = (i === 0 ? '< ' : '> ') + barValue.toLocaleString('fr-FR');
                bar.name = label;
    ...
    result.forEach((res) => {
        res.tooltipProps = Object.assign(Object.assign({}, res.tooltipProps), { tooltipItems: [
                {
...
const generatePriceSurfaceScatter = ({ priceSorted, surfaceSorted, dealsBottomOutliersRemoved }) => {
    const data = [];
    const topOutliersId = [];
    // Remove top outliers when result length  > 10
    if (priceSorted.length > 10) {
        for (let i = 2; i > -1; i--) {
            if (priceSorted[i].valeur_fonciere >
                1.25 * priceSorted[i + 1].valeur_fonciere) {
                topOutliersId.push(...priceSorted.slice(0, i + 1).map((e) => e.id_mutation));
                break;
            }
        }
        for (let i = 2; i > -1; i--) {
            if (surfaceSorted[i].total_surface_reelle_bati >
                1.25 * surfaceSorted[i + 1].total_surface_reelle_bati) {
                topOutliersId.push(...surfaceSorted.slice(0, i + 1).map((e) => e.id_mutation));
                break;
            }
        }
    }
    const deals = dealsBottomOutliersRemoved.filter((deal) => !topOutliersId.includes(deal.id_mutation));
    let sumPpms = 0;
    deals.forEach((deal) => {
...
const generatePricePerMeterIncreaseLine = (deals) => {
    const result = [
    ...
    deals.forEach((deal) => {
        const year = result.find((year) => new Date(deal.date_mutation).getFullYear().toString() ==
            year.name);
    ...
    return result.map((year) => ({
        ...
        tooltipProps: {
            tooltipItems: [
                {
                    name: 'Prix / m²',
                    value: Math.floor(year.sum / year.count).toLocaleString('fr-FR'),
                    unit: ' €'
                },
                {
                    name: 'Nombre de transactions',
...
const generateGraphData = (deals, isClustered) => {
    const pricePerMeterSquareSorted = [...deals].sort((a, b) => b.prix_metre_carre - a.prix_metre_carre);
    const priceSorted = [...deals].sort((a, b) => b.valeur_fonciere - a.valeur_fonciere);
    const surfaceSorted = [...deals].sort((a, b) => b.total_surface_reelle_bati - a.total_surface_reelle_bati);
    ...
    const pricePerMeterMedian = pricePerMeterSquareSorted[Math.floor(pricePerMeterSquareSorted.length / 2)].prix_metre_carre;
    const dealsBottomOutliersRemoved = deals.length > 10
        ? priceSorted.filter((deal) => deal.prix_metre_carre > 0.1 * pricePerMeterMedian)
        : priceSorted;
    const priceSurfaceScatter = isClustered
        ? null
        : generatePriceSurfaceScatter({
            priceSorted: priceSorted,
            surfaceSorted: surfaceSorted,
            dealsBottomOutliersRemoved: dealsBottomOutliersRemoved
        });
    const pricePerMeterIncreaseLine = generatePricePerMeterIncreaseLine(dealsBottomOutliersRemoved);
    return {
        priceHisto,
        pricePerMeterHisto,
        surfaceHisto,
